...
{
	half3 specColor = 0.7;// _SpecColor.rgb;
    #ifdef _SPECGLOSSMAP
...
#ifdef UNITY_STANDARD_USE_STEREO_SHADOW_OUTPUT_STRUCT
VertexOutputStereoShadowCaster vertShadowCaster (VertexInput v)
{
    UNITY_SETUP_INSTANCE_ID(v);

	VertexOutputStereoShadowCaster o;
	UNITY_INITIALIZE_OUTPUT(VertexOutputStereoShadowCaster, o);
    UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(os);
...
VertexOutputShadowCaster vertShadowCaster (VertexInput v)
{
	...
	TRANSFER_SHADOW_CASTER_NOPOS(o, o.pos)
        ...
        #ifdef _PARALLAXMAP
            //TANGENT_SPACE_ROTATION;
			float3 binormal = cross(normalize(v.normal), (v.tangent.xyz)) * v.tangent.w;
